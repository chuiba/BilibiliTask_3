...
发现有部分朋友的定时任务，在第二天没有正常执行。解决办法就是进行一次`push`操作。建议都操作一次。

打开`README.md`，将里面的 😄 删除即可。
...
s2c: true #银瓜子转换硬币，默认转换为true。[true,false]
autoCharge: true #是否允许月底B币卷给自己充电，默认为true允许。[false,true]
``` 
