...
public class BiCoinApply implements Task {
    ...
    public void run() {
        try{
            ...
            /* B币券余额 */
            double couponBalance = Double.parseDouble(userData.getCouponBalance());
    ...
    public void doCharge(Double couponBalance) {
         ...
         */
        if(couponBalance < 2){
            LOGGER.warn("【用B币卷给自己充电】: {}<2 ,无法给自己充电❌", couponBalance);
            return ;
        }
        /* 被充电用户的userID */
        ...
        if (resultCode == 0) {
            ...
            if (statusCode == 4) {
                LOGGER.info("【用B币卷给自己充电】: 本次给自己充值了: {}个电池✔", couponBalance * 10);
                /* 获取充电留言token */
